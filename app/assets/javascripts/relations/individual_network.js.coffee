$ ->
  if $('body').attr('id').match(/comedians-analysis/)
    Network = () ->
      ...
      network.updateSearch = (searchTerm) ->
        ...
        node.each (d) ->
          ...
          if searchTerm.length > 0 and match >= 0
            ...
            d.searched = false
            element.style("fill", (d) -> nodeColors(d.company))
      ...
      setupData = (data) ->
        ...
        countExtent = d3.extent(data.nodes, (d) -> d.appearcount)
        ...
        data.nodes.forEach (n) ->
          ...
          n.radius = circleRadius(n.appearcount)
      ...
      # attr is value stored in node, like 'company'
      ...
      filterNodes = (allNodes) ->
        ...
        if filter == "popular" or filter == "obscure"
          appearcounts = allNodes.map((d) -> d.appearcount).sort(d3.ascending)
          cutoff = d3.quantile(appearcounts, 0.5)
          filteredNodes = allNodes.filter (n) ->
            if filter == "popular"
              n.appearcount > cutoff
      ...
      updateNodes = () ->
        ...
        node.enter().append("circle")
          ...
          .attr("cy", (d) -> d.y)
          .attr("r", (d) -> d.radius)
          .style("fill", (d) -> nodeColors(d.company))
